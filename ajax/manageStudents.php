...
if($type == "add"){
    ...
    if($count_rows && $count_rows[0] > 0){
        ...
        if($sql){
            ...
            $mysqli_connect->query("INSERT INTO tbl_students SET student_fname='$student_fname',student_mname='$student_mname',student_lname='$student_lname',course_id='$course_id', date_added='$date', user_id='$user_id', section='$section', year_level='$year_level'") or die(mysqli_error());
    ...
    if($checker->num_rows > 0){
        ...
        $user_id = $row['user_id'];
        $sql = $mysqli_connect->query("UPDATE tbl_students SET student_fname='$student_fname',student_mname='$student_mname',student_lname='$student_lname', user_id='$user_id', year_level='$year_level', section='$section' WHERE student_id ='$student_id'") or die(mysqli_error());
        if($sql){
            $mysqli_connect->query("UPDATE tbl_users SET user_fname='$student_fname',user_mname='$student_mname',user_lname='$student_lname', username='$username'  WHERE user_id ='$user_id'");
            echo 1;
        }else{
            echo 0;
        }
